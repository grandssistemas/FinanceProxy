...
public class ProxyDocAPI extends AbstractClient{
    ...
    public ResponseEntity post(@RequestBody Map titulo) {
        return this.post("/api/doc/", titulo);
    }

    @RequestMapping(value = "/new", method = RequestMethod.GET)
    public Map initialState() {
        return (Map) this.get("/api/doc/new").getBody();
    }

    @RequestMapping(value = "/{id}", method = RequestMethod.GET)
    public Map load(@PathVariable Long id) {
        return (Map) this.get(String.format("/api/doc/%d", id)).getBody();
    }

    @RequestMapping(method = RequestMethod.GET)
    public Map pesquisa(QueryObject query) throws IOException {
        return (Map) this.get("/api/doc",
    ...
    public Map update(@PathVariable("id") Long id,
        ...
        return (Map) this.put(String.format("/api/doc/%d", id), model).getBody();
    ...
    public Map remove(@PathVariable("id") Long id,
                      @RequestBody Map model) {
        return (Map) this.delete(String.format("/api/doc/%d", id), model).getBody();
    }
