...
public class ProxyCheckoutAPI extends AbstractClient {
    ...
    @Autowired
    public ProxyCheckoutAPI(FinanceProxyProperties fProperties) {
        super();
    ...
    public JsonNode initialState() {
        return (JsonNode) this.get("/api/cashcheckout/new").getBody();
    }

    @RequestMapping(value = "/{id}", method = RequestMethod.GET)
    public JsonNode load(@PathVariable Long id) {
        return (JsonNode) this.get(String.format("/api/cashcheckout/%d", id)).getBody();
    }

    @RequestMapping(method = RequestMethod.GET)
    public JsonNode pesquisa(QueryObject query) throws IOException {
        return (JsonNode) this.get("/api/cashcheckout",
    ...
    public JsonNode update(@PathVariable("id") Long id,
        ...
        return (JsonNode) this.put(String.format("/api/cashcheckout/%d", id), model).getBody();
    ...
    public JsonNode remove(@PathVariable("id") Long id,
                      @RequestBody JsonNode model) {
        return (JsonNode) this.delete(String.format("/api/cashcheckout/%d", id), model).getBody();
